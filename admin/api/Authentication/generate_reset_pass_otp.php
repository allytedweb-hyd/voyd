...
$registerMail = $data->registeredEmail;


if (empty($registerMail)) {
    echo json_encode(["status" => false, "message" => "Email required"]);
    exit();
}

$getCustomerInfo = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM customer WHERE customer_email  = '$registerMail' AND status = 1"));

if ($getCustomerInfo >= 1) {
    $otp = rand(100000, 999999);
    $expires = time() + 1800;
    $otp_str = strval($otp); // convert to string if not already
    $formattedOtp = substr($otp_str, 0, 3) . '-' . substr($otp_str, 3);

    $updateCustomerDetails = mysqli_query($conn, 'UPDATE customer SET otp="' . $otp . '", otp_expires="' . $expires . '" WHERE customer_email ="' . $registerMail . '" AND status=1');
} else {
    http_response_code(404);
    $Cusresponse = ['status' => false, 'message' => 'User Not Found with this email'];
...
try {
    ...
    $mail->addAddress($registerMail);               //Name is optional    
    ...
    $mail->Subject = 'OTP to Rest Password';
...
<body class="body"
                                                            ...
                                                            <tr>
                                                                <td align="center" class="es-text-1448 es-m-p0b"
                                                                    ...
                                                                    <h4 class="es-override-size es-text-mobile-size-16"
                                                                        ...
                                                                        <strong>We received a request to reset your
                                                                            password. Use the following 6-digit OTP to
                                                                            securely complete your password
                                                                            reset.</strong>
                                                                    </h4>
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td align="center" class="es-text-9182"
                                                                    ...
                                                                    <p class="es-text-mobile-size-13 es-override-size"
                                                                        ...
                                                                        If you did not request a password reset, kindly
                                                                        ignore this message. Your existing account
                                                                        credentials will stay safe
                                                                        and unchanged, ensuring no unauthorized access
                                                                        to your account.</p>
                                                                </td>
                                                            </tr>
                                                        </table>
                                                        ...
                                                        <table cellpadding="0" cellspacing="0" role="presentation"
                                                            ...
                                                            <tr>
                                                                <td align="center" class="es-m-p15b"
                                                                    style="padding:15px 10px;Margin:0;padding-bottom:25px">
                                                                    <p class="es-text-mobile-size-13 es-override-size"
                                                                        style="Margin:0;mso-line-height-rule:exactly;font-family:arial, helvetica neue, helvetica,
                                                                                                                                            sans-serif;line-height:21px;letter-spacing:0;color:#333333;font-size:14px">
                                                                        Your OTP will remain valid for the next 30
                                                                        minutes. Please enter and complete verification
                                                                        within this period to
                                                                        continue securely.</p>
                                                                </td>
                                                            </tr>
                                                        </table>
    ...
    $mail->AltBody = 'Otp to reset password';

    $send = $mail->send();

    if ($send) {
        // $stat = ['status' => true, 'message' => '6-digit OTP sent to your registered email successfully'];
        $stat = ['status' => true, 'message' => ' OTP is shared to Registered Email Successfully'];
    }
} catch (Exception $e) {
