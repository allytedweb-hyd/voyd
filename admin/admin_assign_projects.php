                                ...
                                <table class="table table-striped table-bordered" id="example2">
                                    <thead>
                                        <tr class="tableheadingrow">
                                            ...
                                            <th scope="col" class="noExport" id="projectInvoicesShow">Invoice</th>
                                            <th scope="col" class="noExport" id="actionColumn">Action</th>
                                            <th scope="col" class="noExport" id="projectStatus">Status</th>
                                        </tr>
                                    </thead>
                                    <tbody class="quote-table-data page-with-table">
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
            ...
            <div class="modal-body">
                <form method="post" enctype="multipart/form-data" id="assignProjectForm">
                    <div class="form-group">
                        ...
                        <label for="projectUsers" class="control-label">Project Manager:</label>
    ...
    // $getrole = mysqli_query($conn,"SELECT * FROM admin_roles WHERE role_id=2 AND status=2");
    // $getroledetails = mysqli_fetch_array( $getrole );

    $getuser = mysqli_query($conn, "SELECT * FROM login_admin WHERE role_id=2 AND status=2");

   
    if ($getuser) {
        while ($getuserdetails = mysqli_fetch_array($getuser)) {
            ?>
            <option value="<?php echo htmlspecialchars($getuserdetails['id']); ?>">
...
$("#assignProjectForm").on("submit", function (e) {
    ...
    if (projectUser === "") {
        $("#projectUserError").text("Please select a Project Manager.");
        isValid = false;
    }
...
$(document).ready(function () {
    var table = $('#example2').DataTable({
        ...
        dom: '<"top d-flex justify-content-between mb-3"Bf>rt<"bottom d-flex justify-content-between mt-3"ip>',
    ...
    },
   {
    ...
    const columnMap = {};
    $('#example2 thead th').each(function (idx) {
    ...
    function toggleColumns(filterVal) {
        const isAll = (filterVal === 'All' || filterVal === '' );
        ...
        const optionalCols = [
            ...
            'assignedProjectUser',
            'projectStartdate',
            'projectDeadlineHeader',
            'projectStatus'
        ];

        optionalCols.forEach(colId => {
            if (columnMap[colId] !== undefined) {
                table.column(columnMap[colId]).visible(!isAll);
            }
        });

       
        if (columnMap['projectInvoicesShow'] !== undefined) {
            table.column(columnMap['projectInvoicesShow']).visible(true);
        }

      
        if (columnMap['actionColumn'] !== undefined) {
            table.column(columnMap['actionColumn']).visible(true);
        }
    }

    function loadTableData(filterVal) {
        $.ajax({
            url: 'ajax2.php',
            type: 'GET',
            ...
            success: function (data) {
                ...
                const $rows = $('<tbody>').html(data).find('tr');
                $rows.each(function (i) {
                    const rowData = [];
                    $(this).find('td').each(function () {
                        rowData.push($(this).html());
                    });
                    console.log('Row', i, 'td count:', rowData.length, rowData);
                    table.row.add(rowData);
                });

                table.draw();
                toggleColumns(filterVal);
            },
    ...
    const initialFilter = $('#filter').val() || 'All';
    loadTableData(initialFilter);
...
<script>
    $(document).ready(function () {
    if ($('tbody.page-with-table').length > 0) {
        $('#example2_filter').css({
