            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page"><a href="./vendor_testimonials.php">View Vendor Testimonials</li></a>
                    </ol>
                </nav>
            </div>
            ...
            <div class="col-xl-6 mx-auto">
                ...
                <div class="">
                    <div class="card-body new_card_form">
                        <h3 class="mb-4 text-center htext">Update Vendor Testimonials</h3>
                        <?php
                        $query = mysqli_query($conn, "select * from vendor_testimonials where 
                        vendor_testimonial_id='" . $_GET['id'] . "'");
...
<?php
    ...
    $getvendorname = mysqli_query($conn, "SELECT * FROM vendor_management WHERE vendor_id='" . $fetch['name'] . "'");
    $fetchvendorname = mysqli_fetch_array($getvendorname);
    ?>
    ...
    $getActiveVendor = mysqli_query($conn, "SELECT * FROM vendor_management WHERE status = 1 AND vendor_id != '" . $fetchvendorname['vendor_id'] . "'");
    while ($vendorsList = mysqli_fetch_array($getActiveVendor)) {
    ?>
    ...
    ?>
                               ...
                               <div class="col-md-6">
                                <label for="input2" class="form-label"> Testimonial Name<span class="errorindicator">*</span></label>
                                <input type="text" class="form-control" name="testimonial_name" id="testimonial_name" value="<?php echo $fetch['testimonial_name']; ?>">
                                <p id="errText" class="error-text"></p>
                            </div>
...
94 × 
94px, Size< 1 Mb)</span></label>
                                <input type="file" class="form-control" name="image" id="image">
                                <img src="./Uploads/vendortestimonials/<?php echo $fetch['image'] ?>" width="100" height="80" />
                                <input type="hidden" name="oldImage" value="<?php echo $fetch['image'] ?>" />
                                <input type="hidden" name="blogId" value="<?php echo $fetch['vendor_testimonial_id'] ?>" />
                                <p id="errText" class="error-text"></p>
                            </div>
...
<script src="./assets/api/vendortestimonials.js"></script>
